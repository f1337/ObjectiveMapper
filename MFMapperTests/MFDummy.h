//
//  MFDummy.h
//  Empire
//
//  Created by Michael R. Fleet on 12/21/12.
//  Copyright (c) 2012 Michael Fleet. All rights reserved.
//



#import <Foundation/Foundation.h>



@interface MFDummy : NSObject



@property NSArray           *anArray;
@property NSDate            *aDate;
@property NSDecimalNumber   *aDecimalNumber;
@property NSObject          *aNilObject;
@property NSNumber          *aNumber;
@property NSOrderedSet      *anOrderedSet;
@property NSString          *aString;
@property NSDate            *aTimestamp;
@property NSURL             *aURL;



@end
